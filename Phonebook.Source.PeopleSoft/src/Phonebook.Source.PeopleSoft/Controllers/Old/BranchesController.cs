...
namespace Phonebook.Source.PeopleSoft.Controllers.Old {
  ...
  public class BranchesController : ControllerBase {
    ...
    public BranchesController(ModelContext context) { this.context = context; }
      ...
      return this.InlcudeDependencies(context.Rooms)
          .Select(
              b => new {
                  ...
                  LinkRoutingWebsite =
                      $"http://maps.google.de/maps?f=q&hl=de&q={b?.Key?.Name} {b?.Key?.Number},  {b?.Key?.Location?.Name}",
                  ReceptionFax = string.Empty, Description = b?.Key?.Address,
                  ReceptionPhone = string.Empty, LinkPicture = string.Empty,
                  LinkRoutingInfo = string.Empty,
                  City = new {b?.Key?.Location?.Name,
                              Building = $"{b?.Key?.Name} {b?.Key?.Number}",
                              ZipCode = Regex.Replace(b?.Key?.Address,
                                                      ".*, ([0-9]*) .*", "$1")},
                  RoomCollection = b?.Select(
                      d => new {
                          Building =
    ...
    private IEnumerable<IGrouping<Building?, Room>>
    InlcudeDependencies(IQueryable<Room>query) {
