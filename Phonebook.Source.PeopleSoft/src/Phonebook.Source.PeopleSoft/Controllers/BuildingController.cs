...
namespace Phonebook.Source.PeopleSoft.Controllers {
  ...
  public class BuildingController : ControllerBase {
    ...
    public BuildingController(ModelContext context) { Context = context; }
    // GET: api/Building
    [HttpGet]
    public IEnumerable<Building>
    Get() {
    ...
    private IQueryable<Building>InlcudeDependencies(IQueryable<Building>query) {
      return query.AsNoTracking()
          ...
          .Select(b => new Building(){
            ...
            Location = b.Location !=
                       null ? new Location(){
                           Id = b.Location.Id, Country = b.Location.Country,
                           Name = b.Location.Name,
                           ShortName = b.Location.ShortName} : null,
            BuildingParts = b.BuildingParts !=
                            null? b.BuildingParts.Select(
                                bp => new BuildingPart(){
                                    BuildingId = bp.BuildingId, Id = bp.Id,
                                    Description = bp.Description}) : null,
            Floors = b.Floors !=
                     null ? b.Floors.Select(
                         f => new Floor(){
                             Id = f.Id, BuildingId = f.BuildingId,
                             Description = f.Description,
                             Rooms = f.Rooms.Select(
                                 r => new Room(){Id = r.Id, FloorId = r.FloorId,
                                                 Map = r.Map,
