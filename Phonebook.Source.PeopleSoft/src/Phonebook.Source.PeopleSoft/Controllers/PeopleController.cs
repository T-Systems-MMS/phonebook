...
namespace Phonebook.Source.PeopleSoft.Controllers {
  ...
  public class PeopleController : ControllerBase {
    ...
    public PeopleController(ModelContext context) { Context = context; }
    // GET: api/People
    [HttpGet]
    public IEnumerable<Phonebook.Source.PeopleSoft.Models.Old.Person>
    Get() {
      _ = Context.Peoples.ToList();
      return this.InlcudeDependencies(Context.Peoples)
          .ToList()
          .Select(d => new Phonebook.Source.PeopleSoft.Models.Old.Person(d));
    }
    ...
    private IEnumerable<Person>InlcudeDependencies(IQueryable<Person>query) {
      return query.AsNoTracking()
          ...
          .ThenInclude(
              f => f.Building) // we must import also here the building because
                               // sometimes a room hasn't a buildingpart....
          .ThenInclude(b => b.Location)
          .Include(p => p.Room)
          ...
          .Select(
              p =>
                  ...
                  new Person(){
                    ...
                    Function = p.Function !=
                               null ? new Function(){
                                   Id = p.Function.Id, Label = p.Function.Label,
                                   Code = p.Function.Code} : null,
                    LastName = p.LastName,
                    MobilPhone = p.MobilPhone,
                    OrgUnit = p.OrgUnit != null ? CreateOrgUnitTree(p.OrgUnit) :
                                               new OrgUnit(),
                    ...
                    StatusId = p.StatusId,
                    Status = p.Status !=
                             null ? new Status(){Id = p.Status.Id,
                                                Name = p.Status.Name,
    ...
    private Room CreateRoomTree(Room room) {
      ...
      result.BuildingPart = room.BuildingPart != null ? new BuildingPart(){
        ...
        Building = room.BuildingPart.Building != null ? new Building(){
          ...
          Location =
              room.BuildingPart.Building.Location !=
              null ? new Location(){
                  ...
                  Id = room.BuildingPart.Building.Location.Id} : null
        } : new Building()
      } : new BuildingPart();
      result.Id = room.Id;
        ...
        Building = room.Floor.Building != null ? new Building(){
          ...
          Location = room.Floor.Building.Location !=
                     null ? new Location(){
                         ...
                         Id = room.Floor.Building.Location.Id} : null
        } : new Building(),
    ...
    private OrgUnit CreateOrgUnitTree(OrgUnit orgUnit) {
      ...
      result.Parent =
          orgUnit.Parent != null ? CreateOrgUnitTree(orgUnit.Parent) : null;
      ...
      result.OrgUnitToFunctions =
          getOrgUnitFunction(orgUnit.OrgUnitToFunctions);

      return result;
    }

    private IEnumerable<OrgUnitToFunction>
    getOrgUnitFunction(IEnumerable<OrgUnitToFunction>orgUnitToFunctions) {
      foreach(var item in orgUnitToFunctions) {
        yield return new OrgUnitToFunction(){
            FunctionId = item.FunctionId, OrgUnitId = item.OrgUnitId,
            PersonId = item.PersonId,
            Person = new Person(){ShortName = item.Person.ShortName,
