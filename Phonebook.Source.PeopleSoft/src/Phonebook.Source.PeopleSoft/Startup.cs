  ...
  public class Startup {
    ...
    public void ConfigureServices(IServiceCollection services) {
      ...
      if (Configuration.GetValue<bool>("useSeeding")) {
        services.AddDbContext<ModelContext, SeedingContext>(
            options =>
                options.UseInMemoryDatabase("PeopleSoft")
                    .ConfigureWarnings(
                        w => w.Throw(
        ...
        services.AddDbContext<ModelContext>(
            options =>
                options
                    .UseOracle(
                        Configuration.GetConnectionString("PeopleSoftDatabase"),
                        oracleOptionsAction =>
      ...
      if (Configuration.GetValue<bool>("useAnonymos")) {
        if (!Configuration.GetValue<bool>("useSeeding")) {
          throw new NotSupportedException(
              "It is not supported to use anonymos auth and real data! Please also enable 'useSeeding'!");
        }
    ...
    private void AddWsFederation(IServiceCollection services) {
      services
          ...
          .AddWsFederation(options => {
            // MetadataAddress represents the Active Directory instance used to
            // authenticate users.
            options.MetadataAddress = Configuration.GetValue<string>(
                "WsFederationConfig:MetadataAddress");
            ;

            var appId = Configuration.GetValue<string>(
                "WsFederationConfig:ApplicationId");

            // Wtrealm is the app's identifier in the Active Directory instance.
            // For ADFS, use the relying party's identifier, its WS-Federation
            ...
            options.ClaimsIssuer = Configuration.GetValue<string>(
                "WsFederationConfig:ClaimsIssuer");
            options.Events.OnRedirectToIdentityProvider = (c) => {
              var hostname = string.Empty;
              if (string.IsNullOrWhiteSpace(c.Request.Headers["Referer"]) ==
                  false) {
              ...
              c.Response.Headers.Add(new KeyValuePair<string, StringValues>(
                  "Location",
                  new StringValues(c.ProtocolMessage.CreateSignInUrl())));
              c.HandleResponse();
            ...
            // For AAD, use the App ID URI from the app registration's
            // Properties blade:
            // options.Wtrealm =
            // "https://wsfedsample.onmicrosoft.com/63e4796f-9e4e-40f1-928b-d4efd0642d0d";
          })
          .AddCookie();
    }
